...
function detectGrokLeadModel(sessionDir) {
  if (!sessionDir || !fs.existsSync(sessionDir)) return { modelId: null, effort: null };
...
function detectClaudeLeadModel(records) {
  if (!Array.isArray(records)) return { modelId: null, effort: null };
  for (let i = records.length - 1; i >= 0; i--) {
    const rec = records[i];
    const model = (rec && rec.message && rec.message.model) || rec.model || null;
    if (model) return { modelId: model, effort: null };
  }
  return { modelId: null, effort: null };
}
