...
function enqueue(stateDir, leadSid, message, {
  front = false,
  cwd = null,
  access = null,
  model = null,
  ...
  return mutateInbox(stateDir, leadSid, (items) => {
    const item = {
      ...
      cwd: cwd || null,
      access: access || null,
      // The turn that is about to run. meta.json still describes the previous
      // tape until the turn finishes, so the roster must read these instead.
      model: model || null,
...
// Which lead owns this turn. Used when several Grok sessions share a checkout
// and the caller has no GROK_SESSION_ID — the item id is not a guess.
function findLeadByItemId(stateDir, itemId, agentName) {
  if (!itemId || !stateDir || !fs.existsSync(stateDir)) return [];
  let leads;
  try {
    leads = fs.readdirSync(stateDir, { withFileTypes: true });
  } catch {
    return [];
  }
  const hits = [];
  for (const lead of leads) {
    if (!lead.isDirectory()) continue;
    const agentsDir = path.join(stateDir, lead.name, 'agents');
    if (!fs.existsSync(agentsDir)) continue;
    let names;
    try {
      names = fs.readdirSync(agentsDir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const ent of names) {
      if (!ent.isDirectory()) continue;
      if (agentName && ent.name !== agentName) continue;
      const inbox = loadInbox(stateDir, path.join(lead.name, 'agents', ent.name));
      if (inbox.some((it) => it && it.id === itemId)) {
        hits.push({ leadSessionId: lead.name, agentName: ent.name });
      }
    }
  }
  return hits;
