...
function buildArgs({ prompt, effort, model, permissionMode, rules, sessionId, resume, agent }) {
  // No --cwd: the child is spawned in the lead's cwd. --append-system-prompt is
  // the analogue of grok's --rules, and disables system-prompt snapshotting so
  // the persona is re-applied on every resumed turn.
  const base = [
    '-p', prompt,
    '--output-format', 'json',
