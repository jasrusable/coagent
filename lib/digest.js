...
function buildDigestFromRecords(opts) {
  ...
  return {
    markdown: `${header}\n${body}\n`,
    inline: buildInlineFromEvents(fullEvents, side, home),
    side,
    lastMarker,
    isDelta,
    fallbackNote,
...
/** Snapshot + last few user/lead turns, capped — safe to paste into the peer prompt. */
function buildInlineFromEvents(fullEvents, side, home) {
  const snapshot = formatSidecarSummary(side);
  const speech = (fullEvents || []).filter((e) => e.kind === 'user' || e.kind === 'agent');
  let users = 0;
  const kept = [];
  for (let i = speech.length - 1; i >= 0; i--) {
    kept.unshift(speech[i]);
    if (speech[i].kind === 'user') users += 1;
    if (users >= CONTEXT_USER_TURNS) break;
  }
  const timeline = renderTimelineAndSide(kept, home).blocks.join('\n');
  let body = `${snapshot}\n${timeline}`.trim();
  if (body.length > INLINE_MAX_CHARS) {
    body = `…\n${body.slice(body.length - INLINE_MAX_CHARS)}`;
  }
  return body;
}
